/*
*	soSlider 1.0 - simple gallery with jQuery
*	made by bujichong
*	作者：不羁虫
...
;(function($){
	$.fn.soSlider = function (o) {
		var opt = {
			/* slider对象属性 */
			slideType : 'fade',//切换方式 为['left','right','up','slide','slideUpDown','fade','slideUp','slideDown','scrollUp','scrollDown','scrollLeft','scrollRight','scrollUpDown','scrollLeftRight','random']中任意一值，无则直接切换
			slideObj : 'div', //切换子对象 默认为div,如 'img' '.divSlide'
			easing :'swing',//swing,linear
			autoChange: true,//是否自动切换，默认是true
			overStop : true, //鼠标经过时是否停止自动切换
			changeTime :5000, //切换间隙时间 默认是5s
			sliderTime :600, //切换动画时间默认 0.5s
			x:0,//切换对象定位偏移量 x轴
			y:0,//切换对象定位偏移量 y轴
			...
			/* thumb及相关 */
			thumb : null,//自定义切换thumb对象，用来导航slideObj，如 '.ul_thumb li'，
			inThumb : null,//内置thumb 父ul 的 class 名称，一旦设置，自动添加内置thumb，内置的为一个ul li 列表作为thumb，添加inThumb如 'ul_thumb'，只写true，返回slideObj 即为 '.ul_thumb li'
		...
		var _o = _self.find(opt.slideObj),size = _o.length;
		var osW = sW*size,soH = sH*size;
		var now=0,next,z=1*opt.zIndex;
		var autoRun,delayEvent;
		var sIdx = (opt.slideType>-1)?opt.slideType:jQuery.inArray(opt.slideType,['left','right','up','slide','slideUpDown','fade','slideUp','slideDown','scrollUp','scrollDown','scrollLeft','scrollRight','scrollUpDown','scrollLeftRight']);//所有切换方式数组
		if (sIdx==12) {sIdx = 8;}//特殊处理
		if (sIdx==13) {sIdx = 10;}

		var wrap = $('<div style="position:relative;overflow:hidden;width:'+sOutW+'px;height:'+sOutH+'px;"></div>');
		...
		if (opt.btnPrev) {//点击上一个按钮事件
			$(opt.btnPrev).click(function () {
				if (opt.slideType=="scrollUpDown") {sIdx = 8;}//特殊处理
				if (opt.slideType=="scrollLeftRight") {sIdx = 10;}
				_change('prev');
			});
		...
		if (opt.btnNext) {//点击下一个按钮事件
			$(opt.btnNext).click(function () {
				if (opt.slideType=="scrollUpDown") {sIdx = 9;}//特殊处理
				if (opt.slideType=="scrollLeftRight") {sIdx = 11;}
				_change('next');
			});
		...
		function _change(idx) {//对象切换函数
			if(_o.queue().length<1){//防止连续点击
				...
				_thumbChange(next);
				if (opt.slideType=='random') {sIdx= Math.floor(Math.random()*7)+5;}
				switch (sIdx) {
					case 0:
						_self.animate({marginLeft:-next*sW},opt.sliderTime,opt.easing,function () {opt.afterEvent(now,next);});break;
					case 1:
						_self.animate({marginLeft:next*sW-sW*(size-1)},opt.sliderTime,opt.easing,function () {opt.afterEvent(now,next);});break;
					case 2:
						_self.animate({marginTop:-next*sH},opt.sliderTime,opt.easing,function () {opt.afterEvent(now,next);});break;
					case 3:
						_o.eq(now).slideUp(opt.sliderTime);
						_o.eq(next).slideDown(opt.sliderTime,opt.easing,function () {opt.afterEvent(now,next);});
						break;
					case 4:
						_o.eq(now).css({'zIndex':z}).slideUp(opt.sliderTime).siblings().css({'zIndex':0});
						_o.eq(next).css({'zIndex':z}).slideDown(opt.sliderTime,opt.easing,function () {opt.afterEvent(now,next);});
						break;
					case 5:
						_o.eq(now).css({'zIndex':z}).fadeOut(opt.sliderTime).siblings().css({'zIndex':0});
						_o.eq(next).css({'zIndex':z}).fadeIn(opt.sliderTime,opt.easing,function () {opt.afterEvent(now,next);});
						break;
					case 6:
						_o.eq(next).css({'zIndex':z}).show().siblings().css({'zIndex':0});
						_o.eq(now).css({'zIndex':z+1}).slideUp(opt.sliderTime,opt.easing,function () {opt.afterEvent(now,next);});
						break;
					case 7:
						_o.eq(now).css({'zIndex':z}).siblings().css({'zIndex':0});
						_o.eq(next).css({'zIndex':z+1}).hide().slideDown(opt.sliderTime,opt.easing,function () {opt.afterEvent(now,next);});
						break;
					case 8:case 9:
						var wh = (sIdx==8)?-sH:sH;
						_o.eq(next).css({'zIndex':z}).show().siblings().css({'zIndex':0});
						_o.eq(now).css({'zIndex':z+1}).animate({'marginTop':wh},opt.sliderTime,opt.easing,function () {
					...
					case 10:case 11:
						...
						_o.eq(next).css({'zIndex':z}).show().siblings().css({'zIndex':0});
						_o.eq(now).css({'zIndex':z+1}).animate({'marginLeft':ww},opt.sliderTime,opt.easing,function () {
	...
	$(function () {
		if ($('.soSlider').length) {//针对.soSlider自动加载切换事件
			$('.soSlider').each(function () {
				$(this).soSlider();
			});
		}
	})
