  ...
  $.extend($rules, {
    ...
    fnValid : {
        validator: function (value, param) {//函数验证
          if (param && param[1]) $rules.fnValid.message = param[1];//自定义提示
          if(window[param[0]]){
...
function isIdCardNo(code,opt){
  ...
  if (opt) {
        ...
        var $sex = $('#'+optArr[1]);
        var $age = $('#'+optArr[2]);
        var datebox = $bir.hasClass('easyui-datebox');
        var sexbox = $sex.hasClass('easyui-combobox');
        var agebox = $age.hasClass('easyui-numberbox');
        if (pass) {
          var birthday = GetBirthday(code,$age,agebox);

        if(datebox){
          $bir.datebox('setValue',birthday);
        }else{
          $bir.val(birthday);
        }
        if(sexbox){
          $sex.combobox('setValue',Getsex(code));
        }else{
          $sex.val(Getsex(code));
        }
...
function GetBirthday(psidno,$age,agebox){
