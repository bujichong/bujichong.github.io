...
//   // window.console && console.log(evt, request, settings);
//   window.console && console.log( $.parseJSON(request.responseText));
// });
var $ajax = {//统一的异步post请求
    alertErr : function (rst,msg) {
      ...
      if(rst.timeout&&rst.timeout===301){
    	...
    	var msg = rst.msg || msg ||'信息请求失败';
            ...
            if (errDatails) {
                showErr = '<i class="layui-layer-ico layui-layer-ico2"></i><h2 class="h2-err">对不起！'+ msg +'</h2><p>请检查网络或发送邮件到管理员邮箱 <br /><a class="a-mail" href="mailto:servicer@aierchina.com">servicer@aierchina.com</a></p><span class="s-errDetails">查看错误信息</span> <div class="errPopInfoBox"><div class="errPopInfo">'+errDatails+'</div></div>';
            };
    	layer.open({
    	  type: 1,
    	  skin:'soerrPop',
    ...
    post: function (url, data, tip, jsonData,opt) {
        ...
        // window.console && console.log($.type(url),$.type(data),$.type(tip),$.type(jsonData),$.type(opt));
        //简单重载参数
        var inOpt = {};
        ...
        // window.console && console.log(url, data, tip,jsonData,inOpt);
        ...
        if (o.tip) {//提示
          ...
          var event = function (dtd) {
            ...
            layer.confirm(msg, {icon: 0, title:false,btnAlign: 'c'}, function(index){
              if(canAjax){
                canAjax = false;
                $.ajax({
                  ...
                  success: function (rst) {
                    ...
                    if (rst) {
                      ...
                      if (rst.code==='200'|| rst.code==='201') {
                        o.success(rst);
                        if(o.calltip){$pop.msg.success(msg,{time: 1500});}
                      } else {
                        $ajax.alertErr(rst);
                      }
                    }
                    window.console&&console.log(dtd);
                    dtd.resolve(rst);
                  },
          ...
          return $.when(event(dtd));
            ...
            dtd = $.ajax({
              ...
              success: function (rst) {
                ...
                o.callback(rst);
                if(rst){
                  if(rst.code==='200'){o.success(rst);return;}
                  if (rst.code==='201') {
                    if(o.calltip){
                      var msg = rst.msg ||'';
                      $pop.msg.success(msg,{time: 1500});
                    }
                    o.success(rst);
                  } else {
                    $ajax.alertErr(rst);
